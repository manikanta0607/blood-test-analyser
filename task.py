...
from my_tools import BloodTestReportTool
# from crewai_tools import BaseTool
from crewai.tools import BaseTool


# ✅ Tool wrapper that conforms to CrewAI's expectations
class ReadBloodReportTool(BaseTool):
    name = "ReadBloodReportTool"
    description = "Reads and returns data from a blood report PDF."

    def __init__(self, reader):
        self.reader = reader

    async def run(self, *args, **kwargs):
        return await self.reader.read_data_tool()

    async def _run(self, *args, **kwargs):
        return await self.run(*args, **kwargs)

# ✅ Instantiate your tool and wrap it
blood_test_report_tool = BloodTestReportTool()
read_data_tool_instance = ReadBloodReportTool(blood_test_report_tool)
...
Feel free to contradict yourself within the same response.""",
    agent=doctor,
    tools=[read_data_tool_instance],
    async_execution=False,
)

nutrition_analysis = Task(
    description="Look at some blood data and tell them what to eat or not eat.\n"
